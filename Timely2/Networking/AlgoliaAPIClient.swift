...
class AlgoliaAPIClient {
    ...
    func fetchStories(from urlComponents: URLComponents, page: Int,
                      completion: @escaping (Result<[Story], HackerNewsError>) -> Void) {
        
        let pageQueryItem = URLQueryItem(name: "page", value: String(page))
        var newUrlComponents = urlComponents
        newUrlComponents.addOrModify(pageQueryItem)
        
        guard let url = newUrlComponents.url else {
